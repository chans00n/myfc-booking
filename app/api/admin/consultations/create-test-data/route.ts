  ...
  try {
    ...
    for (let i = 0; i < insertedAppointments.length; i++) {
      ...
      const consultation = {
        ...
        consultation_type: ['video', 'phone', 'in_person'][i % 3] as 'video' | 'phone' | 'in_person',
        ...
        consultation_notes: dateInfo.status === 'completed' 
          ? `Test consultation completed successfully. Client is motivated and ready to start their fitness journey.`
          : null,
        started_at: dateInfo.status === 'completed' ? addHours(new Date(appointment.appointment_date + ' ' + appointment.start_time), 0).toISOString() : null,
        completed_at: dateInfo.status === 'completed' ? addHours(new Date(appointment.appointment_date + ' ' + appointment.start_time), 1).toISOString() : null,
      }
