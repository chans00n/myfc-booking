...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!consultationService) {
      ...
      if (serviceError) {
        return NextResponse.json({ error: 'Failed to create consultation service' }, { status: 500 })
      }
    ...
    const notificationResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/notifications/consultation`, {
      method: 'POST',
      headers: {
