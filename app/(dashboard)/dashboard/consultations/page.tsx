...
import { format, formatDistanceToNow, startOfDay, endOfDay, isToday, isTomorrow, addDays } from 'date-fns'
import { Video, Phone, Users, Calendar, Clock, Search, ChevronRight, Play, X, RefreshCw, FileText, MessageSquare, TrendingUp, AlertCircle } from 'lucide-react'
...
export default function ConsultationsPage() {
  ...
  const fetchConsultations = async () => {
    try {
      ...
      const mappedData = consultationsData?.map(consultation => {
        ...
        if (!appointment && consultation.appointment_id) {
          console.warn(`Consultation ${consultation.id} references appointment ${consultation.appointment_id} which was not found`)
        }
  ...
  const calculateStats = (consultationData: Consultation[]) => {
    ...
    const completedToday = todayConsultations.filter(c => c.consultation_status === 'completed').length
    
    const upcomingWeek = consultationData.filter(c => {
      if (!c.appointment || !c.appointment.appointment_date) return false
      const appointmentDateTime = new Date(`${c.appointment.appointment_date}T${c.appointment.start_time}`)
      return appointmentDateTime > today && appointmentDateTime <= weekEnd && c.consultation_status === 'scheduled'
    }).length

    const allCompleted = consultationData.filter(c => c.consultation_status === 'completed')
    const completionRate = consultationData.length > 0 
      ? (allCompleted.length / consultationData.length) * 100 
      : 0
    ...
    const noShowRate = consultationData.length > 0 
      ? (noShows / consultationData.length) * 100 
      : 0
  ...
  const filteredConsultations = consultations.filter(consultation => {
    ...
    const clientName = `${consultation.client?.first_name || ''} ${consultation.client?.last_name || ''}`.toLowerCase()
  ...
  const recentConsultations = filteredConsultations.filter(c => 
    c.consultation_status === 'completed' || c.consultation_status === 'cancelled'
              ...
              <TabsContent value="recent" className="space-y-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      All consultations ordered by creation date
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {loading ? (
                            ...
                            <TableRow key={consultation.id}>
                              <TableCell>
                                <div>
                                  <p className="font-medium">
                                    {consultation.client?.first_name || ''} {consultation.client?.last_name || consultation.client?.email || 'Unknown Client'}
                                  </p>
                              ...
                              <TableCell>
                                {consultation.appointment?.appointment_date ? (
                                  <div>
                                    <p>{format(new Date(consultation.appointment.appointment_date), 'MMM d, yyyy')}</p>
                                    <p className="text-sm text-muted-foreground">
                                      {consultation.appointment.start_time ? format(new Date(`${consultation.appointment.appointment_date}T${consultation.appointment.start_time}`), 'h:mm a') : 'N/A'}
                                    </p>
                                  </div>
                                ) : (
                                  'No appointment'
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                                  <span>{consultation.appointment?.start_time && consultation.appointment?.appointment_date ? format(new Date(`${consultation.appointment.appointment_date}T${consultation.appointment.start_time}`), 'h:mm a') : 'N/A'}</span>
                                  <span>•</span>
                        ...
                        <TableBody>
                          {upcomingConsultations.map((consultation) => (
                            <TableRow key={consultation.id}>
                              ...
                              <TableCell>
                                <div>
                                  <p>{consultation.appointment?.appointment_date ? format(new Date(consultation.appointment.appointment_date), 'MMM d, yyyy') : 'N/A'}</p>
                                  <p className="text-sm text-muted-foreground">
                                    {consultation.appointment?.start_time ? format(new Date(`${consultation.appointment.appointment_date}T${consultation.appointment.start_time}`), 'h:mm a') : 'N/A'}
                                  </p>
                                </div>
                              ...
                              <TableCell>{consultation.appointment?.service?.name || 'N/A'}</TableCell>
                              <TableCell>
                                <Button
                                  size="sm"
                                  variant="ghost"
              ...
              <TabsContent value="history" className="space-y-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Recent completed and cancelled consultations
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {loading ? (
                          ...
                          {recentConsultations.map((consultation) => {
                            ...
                            return (
                              <TableRow key={consultation.id}>
                                ...
                                <TableCell>
                                  {consultation.appointment?.appointment_date ? format(new Date(consultation.appointment.appointment_date), 'MMM d, yyyy') : 'N/A'}
                                </TableCell>
                                <TableCell>
                                  <div className="flex items-center gap-2">
                                    {getConsultationIcon(consultation.consultation_type)}
