...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const completedCount =
        recentAppointments?.filter((a) => a.status === "completed").length || 0;
      ...
      const consultationStats = {
        ...
        completionRate:
          weekConsultations?.length > 0
            ? (weekConsultations.filter((c) => c.consultation_status === "completed").length /
                weekConsultations.length) *
              100
            : 0,
      };
      ...
      const [{ data: clients }, { data: services }, { data: payments }] = await Promise.all([
        ...
        supabase
          .from("payments")
          .select("appointment_id, status, created_at, amount_cents")
          .in("appointment_id", activityIds),
      ]);
      ...
      const formattedUpcoming: UpcomingAppointment[] =
        upcoming?.map((apt) => {
  ...
  const statsData = [
    ...
    },
    {
      ...
      icon: DollarSign,
      description:
        stats.weeklyRevenue.percentChange > 0
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={async () => {
                          const res = await fetch("/api/admin/test-appointments", {
                            method: "POST",
                          });
                          if (res.ok) {
                            fetchDashboardData();
                    ...
                    {loading ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Activities will appear here when appointments are created, updated, or
                          completed.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        {activities.map((activity, index) => (
                          <div
                            key={activity.id}
                            className={`flex items-center justify-between py-2 ${index < activities.length - 1 ? "border-b" : ""}`}
                          >
