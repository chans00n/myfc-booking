...
export async function scheduleConsultationNotifications(data: ConsultationNotificationData) {
  ...
  try {
    ...
    if (notificationPrefs.consultation_15min_reminder && notificationPrefs.sms_enabled && data.clientPhone) {
    ...
    if (notificationPrefs.consultation_followup && notificationPrefs.email_enabled) {
      ...
      notifications.push({
        ...
        metadata: {
          ...
          special_offer_description: 'Save 20% on your first massage appointment when you book within 7 days!',
...
export async function sendConsultationNotification(
  ...
  switch (notificationType) {
    ...
    case 'consultation_15min_reminder':
      if (data.clientPhone) {
        const message = data.consultationType === 'video' 
