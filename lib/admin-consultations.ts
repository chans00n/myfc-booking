...
export async function completeConsultation(consultationId: string, notes?: string) {
  const supabase = createClient();
...
export async function cancelConsultation(consultationId: string, reason?: string) {
  const supabase = createClient();
