...
export async function createPaymentIntent({
  ...
  if (!stripe) {
    return { error: 'Stripe is not configured. This function must be called from server-side code.' }
  }
  ...
  try {
    ...
    const paymentIntent = await stripe!.paymentIntents.create({
      ...
      description: description || `Payment for ${appointment.service.name} on ${appointment.appointment_date}`,
      metadata: {
        appointment_id,
    ...
    if (paymentError) {
      ...
      return { error: `Failed to create payment record: ${paymentError.message} (code: ${paymentError.code})` }
    }
