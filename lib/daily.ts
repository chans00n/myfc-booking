...
class DailyService {
  ...
  constructor() {
    ...
    if (!this.domain) {
      console.warn(
        "Daily.co domain not configured. Add NEXT_PUBLIC_DAILY_DOMAIN to your .env.local file"
      );
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 && error.error === "authorization-header-error") {
          throw new Error(
            "Daily.co API key is invalid or missing. Please check your DAILY_API_KEY in .env.local"
          );
        }
  ...
  async getConsultationRoomInfo(roomName: string): Promise<{
    ...
    try {
      ...
      const participantsResponse = await fetch(`${this.baseUrl}/rooms/${roomName}/presence`, {
        headers: this.getHeaders(),
      });
