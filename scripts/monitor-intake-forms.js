...
async function monitorIntakeForms() {
  ...
  console.log(
    "Submitted:",
    form.submitted_at ? new Date(form.submitted_at).toLocaleString() : "Not submitted"
  );
  ...
  if (form.appointment) {
    ...
    console.log(
      "Appointment Date:",
      new Date(form.appointment.appointment_date).toLocaleDateString()
    );
  ...
  if (!clientError && clientForms) {
    ...
    clientForms.forEach((f, index) => {
      console.log(
        `${index + 1}. ${f.id} - Status: ${f.status}, Created: ${new Date(f.created_at).toLocaleDateString()}`
      );
    });
  }
