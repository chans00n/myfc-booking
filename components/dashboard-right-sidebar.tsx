...
export function DashboardRightSidebar({
  upcomingAppointments: defaultAppointments,
  ...
  const fetchAppointmentsForDate = async (date: Date) => {
    ...
    try {
      ...
      const formatted: UpcomingAppointment[] =
  ...
  return (
    <div className="space-y-6">
      {/* Calendar */}
      <DashboardCalendar onDateSelect={handleDateSelect} selectedDate={selectedDate} />
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between">
            ...
            {selectedDate && (
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={clearSelectedDate}>
                <X className="h-3 w-3" />
              </Button>
            )}
          </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {displayAppointments.map((appointment, index) => (
                <div
                  key={appointment.id}
                  className={`py-2 ${index < displayAppointments.length - 1 ? "border-b" : ""}`}
                >
