    ...
    try {
      ...
      if (appointments && appointments.length > 0) {
        ...
        const [{ data: clients }, { data: services }] = await Promise.all([
          supabase.from('profiles').select('id, first_name, last_name').in('id', appointmentClientIds),
        ...
        appointments.forEach(apt => {
          ...
          searchResults.push({
            ...
            subtitle: apt.appointment_date ? format(new Date(apt.appointment_date), 'MMM d, yyyy') : '',
      ...
      if (consultations && consultations.length > 0) {
        ...
        consultations.forEach(consultation => {
          const appointment = consultationAppointments?.find(a => a.id === consultation.appointment_id)
          const client = appointment ? consultationClients?.find(c => c.id === appointment.client_id) : null
          ...
          searchResults.push({
            ...
            subtitle: consultation.created_at ? format(new Date(consultation.created_at), 'MMM d, yyyy') : '',
          ...
          {!loading && search.length === 0 && (
            <>
              ...
              <CommandGroup heading="Quick Actions">
                <CommandItem onSelect={() => { router.push('/dashboard/appointments/new'); setOpen(false) }}>
                  <Calendar className="mr-2 h-4 w-4" />
                  Create New Appointment
                </CommandItem>
                <CommandItem onSelect={() => { router.push('/dashboard/clients/new'); setOpen(false) }}>
                  <User className="mr-2 h-4 w-4" />
                  Add New Client
                </CommandItem>
                <CommandItem onSelect={() => { router.push('/dashboard/consultations'); setOpen(false) }}>
