...
export function BookingWizard() {
  ...
  const { bookingData, currentStep, setCurrentStep, canProceedToStep, resetBooking } = useBooking();
  ...
  const effectiveStep =
    currentStep === 7 && bookingData.paymentPreference !== "pay_now" ? 6 : currentStep;
  ...
  return (
    <div className="w-full">
      <div className="pb-24 sm:pb-0">
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0 mb-3">
            ...
            <span className="text-sm text-muted-foreground">
              Step{" "}
              {currentStep === 7 && bookingData.paymentPreference !== "pay_now" ? "6" : currentStep}{" "}
              of {bookingData.paymentPreference === "pay_now" ? "7" : "6"}
            </span>
          </div>
        ...
        {/* Step Content */}
        <div className="mb-6 sm:mb-8 min-h-[300px]">{renderStep()}</div>
        ...
        {currentStep < STEPS.length && (
          <div className="hidden sm:flex sm:justify-between gap-3">
            ...
            <Button onClick={handleNext} disabled={!isValidStep} size="lg">
              {currentStep === 5 && bookingData.paymentPreference !== "pay_now"
                ? "Complete Booking"
                : currentStep === 6
                  ? "Complete Booking"
                  : "Next"}
      ...
      {currentStep < STEPS.length && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/95 backdrop-blur-sm border-t shadow-lg sm:hidden z-50">
          <div className="flex gap-3 max-w-4xl mx-auto">
            ...
            <Button onClick={handleNext} disabled={!isValidStep} className="flex-1" size="lg">
              {currentStep === 5 && bookingData.paymentPreference !== "pay_now"
                ? "Complete"
                : currentStep === 6
                  ? "Complete"
