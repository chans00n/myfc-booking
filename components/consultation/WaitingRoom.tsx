    ...
    <div className="h-full flex items-center justify-center p-4 bg-gradient-to-br from-primary/5 to-background">
      <Card className="max-w-2xl w-full">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            with {consultation.appointment.client.first_name} {consultation.appointment.client.last_name}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <div className={`flex items-center gap-2 justify-center p-4 rounded-lg ${
            timeStatus.status === 'ready' ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400' :
            timeStatus.status === 'early' ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' :
            'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400'
          }`}>
            {timeStatus.status === 'ready' ? <CheckCircle className="h-5 w-5" /> :
             timeStatus.status === 'early' ? <Clock className="h-5 w-5" /> :
