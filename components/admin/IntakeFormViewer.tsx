    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          <p className="text-sm">
            {form.submitted_at
              ? format(new Date(form.submitted_at), "MMM d, yyyy h:mm a")
              : "Not submitted"}
          </p>
        </div>
      </div>
              ...
              <div>
                ...
                <p>
                  {(form.client as any)?.first_name && (form.client as any)?.last_name
                    ? `${(form.client as any).first_name} ${(form.client as any).last_name}`
                    : (form.client as any)?.first_name ||
                      (form.client as any)?.last_name ||
                      "Not provided"}
                </p>
              </div>
              <div>
                ...
                <p>
                  {(form.client as any)?.date_of_birth
                    ? format(new Date((form.client as any).date_of_birth), "MMM d, yyyy")
                    : "Not provided"}
                </p>
              </div>
            </div>
                  ...
                  <li key={i} className="border-l-2 pl-3">
                    ...
                    {condition.yearDiagnosed && (
                      <p className="text-sm text-muted-foreground">
                        Diagnosed: {condition.yearDiagnosed}
                      </p>
                    )}
                    {condition.currentlyTreated && (
                      <p className="text-sm text-green-600">Currently being treated</p>
                    )}
              ...
              <ul className="mt-1 space-y-2">
                {form.injuries.map((injury: any, i: number) => (
                  <li key={i} className="border-l-2 pl-3">
                    ...
                    {injury.date && (
                      <p className="text-sm text-muted-foreground">Date: {injury.date}</p>
                    )}
                    {injury.currentlyAffects && (
                      <p className="text-sm text-orange-600">Still affects today</p>
                    )}
                ...
                {form.pain_areas.map((area: any, i: number) => (
                  <li key={i} className="border-l-2 pl-3">
                    <p className="font-medium">
                      {area.area} - Level {area.painLevel}/10
                    </p>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <p className="whitespace-pre-wrap">
              {form.treatment_goals || form.goals || "Not specified"}
            </p>
          </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 border rounded p-2 bg-muted/10">
                <img src={form.consent_signature} alt="Client signature" className="max-h-24" />
              </div>
              <p className="text-sm text-muted-foreground mt-1">
                Signed on:{" "}
                {form.consent_date
                  ? format(new Date(form.consent_date), "MMM d, yyyy h:mm a")
                  : "Unknown"}
              </p>
            </div>
          </CardContent>
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p>
                {form.updated_at
                  ? format(new Date(form.updated_at), "MMM d, yyyy h:mm a")
                  : "Never"}
              </p>
            </div>
            {form.reviewed_at && (
